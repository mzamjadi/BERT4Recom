...
flags.DEFINE_string("signature", 'default', "signature_name") #Defines default values for parameters if not given
...
def write_instance_to_example_files(instances, max_seq_length,
    ...
    txt_out = open("output_file.txt","a")#append mode 
 
    for (inst_index, instance) in enumerate(instances):
        try:
            input_ids = vocab.convert_tokens_to_ids(instance.tokens) #####it actually converts tokens to id based on freq######
        except:
            print(instance)
        ...
        input_ids += [0] * (max_seq_length - len(input_ids)) ########creating zero padding
        input_mask += [0] * (max_seq_length - len(input_mask)) ######creating masking for padding
        ...
        masked_lm_weights = [1.0] * len(masked_lm_ids) ##weights for cloz masked only
...
def create_training_instances(all_documents_raw,
    ...
    if force_last:
        ...
        for user, item_seq in all_documents_raw.items():
            ...
            if len(item_seq) <= max_num_tokens:
                ...
                beg_idx.append(0)
                all_documents[user] = [item_seq[i:i + max_num_tokens] for i in beg_idx[::-1]] #creates list of list for each user with seq_len > max_seq_len
    ...
    if force_last:
        ...
        for i, user in enumerate(all_documents):
            instances.extend(
                mask_last(
                    all_documents, user, max_seq_length, short_seq_prob,
...
def mask_last(
    ...
    instances = []
    info = [int(user.split("_")[1])] #user names
    vocab_items = vocab.get_items()

    for tokens in document: #list of sequences for a user
...
def create_instances_from_document_train(
    ...
    for tokens in document:#could be a user with multiple sequences
        assert len(tokens) >= 1 and len(tokens) <= max_num_tokens 
...
def main():
    ...
    dataset = data_partition(output_dir+dataset_name+'.txt') #data_partition is a function in util.py
    [user_train, user_valid, user_test, usernum, itemnum] = dataset #dataset has 5 components, first 3 are dict.
    cc = 0.0
    max_len = 0
    min_len = 100000
    for u in user_train:
    ...
    } #merge train, valid and test
    rng = random.Random(random_seed)

    vocab = FreqVocab(user_test_data) #convert the indx of items based on their popularity
