...
class FreqVocab(object):
    ...
    def __init__(self, user_to_list):
        ...
        #first items
        for token, count in self.counter.most_common(): #converting tokens (previous item ids) to ids based on their popularity
...
#             self.token_to_ids[user] = len(self.token_to_ids) + 1
        ...
        self.vocab_words = list(self.token_to_ids.keys()) # list of tokens (previous ids)
